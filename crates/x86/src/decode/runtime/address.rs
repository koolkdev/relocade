...
pub(super) fn decode(
    ...
    modrm: &Val<I8>,
    complete: impl Fn(FunctionBuilder<'_>, RuntimeCursor, Address32<Val<I32>>) -> Result<(), BuildError>,
) -> Result<(), BuildError> {
    let mode = modrm.unsigned().shr(6);
    let rm = modrm.and(7).unsigned().extend::<I32>();
    let complete_layout =
        |mut body: FunctionBuilder<'_>, mut cursor: RuntimeCursor, sib: Option<Val<I8>>| {
            let base = match &sib {
