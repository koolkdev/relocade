use super::*;
use crate::{PrivilegeLevel, SegmentPermissions};
            ...
            for present in [false, true] {
                for (limit, default_size) in [
                    ...
                    for selector in 0x24..=0x27 {
                        assert_eq!(
                            tables.user_segment_permissions(selector),
                            SegmentPermissions {
                                readable: mask & 1 != 0,
                                writable: mask & 2 != 0,
                            },
...
fn null_missing_and_removed_slots_deny_both_permissions_but_local_zero_is_valid() {
    let mut tables = DescriptorTables::default();
    let none = SegmentPermissions {
        readable: false,
        writable: false,
    };
    let both = SegmentPermissions {
        readable: true,
        writable: true,
    };
    ...
    for selector in 0..4 {
        assert_eq!(tables.user_segment_permissions(selector), none);
    }
    for slot in [4, 0x20, 0xfff8, 0xfffc] {
        for rpl in 0..4 {
            assert_eq!(tables.user_segment_permissions(slot | rpl), both);
        }
    }
    ...
    tables.remove(0x23);
    assert_eq!(tables.user_segment_permissions(0x20), none);
}
...
fn a_nonpresent_descriptor_can_pass_verification_and_still_fail_a_load() {
    ...
    assert_eq!(
        tables.user_segment_permissions(0x27),
        SegmentPermissions {
            readable: true,
            writable: true
        }
