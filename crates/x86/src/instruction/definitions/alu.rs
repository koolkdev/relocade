...
const fn alu_family(first_opcode: u8, handlers: IntegerHandlers<Handler>) -> AluFamily {
    ...
    AluFamily {
        register_rm: [
            register_rm(first_opcode, byte, RegisterRole::Source),
            register_rm(first_opcode + 1, handlers.sized, RegisterRole::Source),
            register_rm(first_opcode + 2, byte, RegisterRole::Destination),
            register_rm(first_opcode + 3, handlers.sized, RegisterRole::Destination),
        ],
...
const TEST_MODRM_FORMS: [Form; 4] = [
    register_rm(0x84, SizedHandlers::fixed(TEST.byte), RegisterRole::Source),
    register_rm(0x85, TEST.sized, RegisterRole::Source),
    rm_immediate(
        0xf6,
        0,
        ImmediateWidth::Byte,
