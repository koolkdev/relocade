...
fn named_writes_coalesce_without_crossing_indexed_writes() {
    ...
    state.write_register(&body, Gpr32::Eax, 0).unwrap();
    state.write_register(&body, Gpr32::Eax, 1).unwrap();
    state
        .write_register(&body, Register32::indexed(index), 2)
        .unwrap();
    state.write_register(&body, Gpr32::Eax, 3).unwrap();
    state.write_register(&body, Gpr32::Eax, 4).unwrap();
...
fn publishing_an_exit_keeps_pending_writes_for_the_continuation() {
    ...
    let mut state = State::new(memory);
    state.write_register(&body, Gpr32::Eax, 42).unwrap();
    body.if_(stop, |mut branch| {
        state.publish(&mut branch, 0x1005, 1)?;
        branch.return_(-1)
    })
    .unwrap();
    state.write_register(&body, Gpr32::Eax, 43).unwrap();
    state.write_register(&body, Gpr32::Ecx, 99).unwrap();
