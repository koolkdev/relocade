...
fn execute(flags: &[&str]) {
    ...
    memory::check_memory(flags, &step);
    memory::check_logical_memory(flags, &step);
    faults::check_faults(flags, &step);
}

#[test]
#[ignore = "requires Node.js; run the explicit V8 lane"]
fn binary_operands_execute_in_v8() {
    execute(&[]);
}

#[test]
#[ignore = "requires Node.js; run the explicit V8 lane"]
fn binary_operands_execute_in_optimizing_v8() {
    execute(&[
        "--no-liftoff",
