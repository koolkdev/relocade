...
fn check_mixed_register_views(flags: &[&str], step: &ModuleFile) {
    let code = [
...
fn check_addressed_words(flags: &[&str], step: &ModuleFile) {
...
fn check_page_boundaries(flags: &[&str], step: &ModuleFile) {
    ...
    for (opcode, fault) in [(0xa1, 0x0004_0000_ffff_ffff), (0xa3, 0x0004_0002_ffff_ffff)] {
    ...
    for mapped in [false, true] {
        ...
        let last = if mapped {
            ...
            Step {
                cpu: &[],
                ram: &[],
                exit: Exit::Fault(0x0004_0000_8000_4000),
            }
        };
        both(
            step,
