...
fn check_replacing_arithmetic(flags: &[&str], step: &ModuleFile) {
    let code = [
...
fn check_replacing_logic(flags: &[&str], step: &ModuleFile) {
    let code = [
...
fn execute(flags: &[&str]) {
    ...
    check_replacing_arithmetic(flags, &step);
    check_replacing_logic(flags, &step);
    for case in LOGICAL {
...
fn logical_results_and_conditions_execute_in_v8() {
    execute(&[]);
}

#[test]
#[ignore = "requires Node.js; run the explicit V8 lane"]
fn logical_results_and_conditions_execute_in_optimizing_v8() {
    execute(&[
        "--no-liftoff",
