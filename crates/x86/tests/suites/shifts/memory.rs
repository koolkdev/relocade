...
#[test]
fn memory_shifts_use_exact_widths_and_old_address_registers() {
    struct Case {
        name: &'static str,
        code: &'static [u8],
        operation: Operation,
    ...
    for case in [
        Case {
            name: "implicit byte shift needs only the last mapped byte",
            code: &[0xd0, 0x23],
        ...
        Case {
            name: "dword arithmetic shift crosses noncontiguous pages",
            code: &[0xc1, 0x3b, 31],
        ...
        Case {
            name: "large arithmetic byte shift retains a positive sign",
            code: &[0xc0, 0x3b, 255],
            operation: Operation::Sar,
            bits: 8,
            count: 255,
        ...
        expected(case.operation, case.bits, case.input, case.count).apply_flags(&mut cpu);
...
#[test]
fn full_write_access_is_required_before_any_shift_or_flag_effect() {
    struct Case {
        name: &'static str,
        code: &'static [u8],
        count: u8,
    ...
    for case in [
        ...
        Case {
            name: "nonzero shift checks a read-only second dword page",
            code: &[0xd1, 0x23],
            count: 1,
            address: 0x4ffe,
            first_writable: Some(true),
